...
struct StoreView:View {
    ...
    @State var storeinfo: StoreDetailResponse = .init(storeName: "", category: "", imageUrl: nil, keywords: [], tags: [:], status: nil, phoneNumber: nil, address: "", time: "", imageCount: 0, x: 0.0, y: 0.0, isWishlist: false)
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            if isActive {
                StoreReviewWrite(showReviewWriteView: $isActive, storeId: storeId)
            } else {
                ScrollView {
                ...
                VStack(alignment: .center, spacing: 8) {
                    HStack(alignment: .top, spacing: 8) {
                        //하트
                        Button(action: {
                            storeVM.toggleWishList(storeId: storeId, isLike: !isWishList)
                            isWishList.toggle()
                        }) {
        ...
        .onAppear {
            ...
            storeVM.storeInfoSuccess .sink { _ in
                ...
                isWishList = storeinfo.isWishlist
