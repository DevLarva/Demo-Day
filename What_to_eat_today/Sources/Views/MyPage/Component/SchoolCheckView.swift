    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack {
                    ...
                    Text("\(1 / 60):\(timeRemaining % 60, specifier: "%02d")")
                        .foregroundColor(.PrimaryOrange300)
                ...
                Spacer()
                    ...
                    CustomNavLink(destination: SchoolSelectView(registerData: $registerData, isLogged: $isLogged)
                        .customNavigationTitle(""), isActive: $isActive) {
                    Button {
                        // 이메일 전송 함수 호출
...
struct SchoolCheckView_Previews: PreviewProvider {
    @State static var test = RegisterRequest.init()
    @State static var EmailCert = ""
    @State static var isLoggedTest: Bool? = false
    static var previews: some View {
        SchoolCheckView(registerData: $test, EmailCert: $EmailCert, isLogged: $isLoggedTest)
    }
}
